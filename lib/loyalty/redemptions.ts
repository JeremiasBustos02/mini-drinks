...
export async function getAvailableLoyaltyBalance(customerProfileId: string) {
  return (await getLoyaltyBalanceSummary(customerProfileId)).availablePoints;
}
...
export async function reserveLoyaltyRedemption(
  ...
  if (!input.requestedPoints)
    return { points: 0, discountCents: 0, settings: null };
  if (!input.customerProfileId)
    throw new Error("Guests cannot redeem loyalty points.");
  const settings = await loadLoyaltyRedemptionSettings(tx);
  const [created] = await tx
    .insert(loyaltyAccounts)
    .values({ customerProfileId: input.customerProfileId })
  ...
  const accountId =
    ...
    (
      await tx
        ...
        .from(loyaltyAccounts)
        .where(eq(loyaltyAccounts.customerProfileId, input.customerProfileId))
        .limit(1)
    )[0]?.id;
  if (!accountId) throw new Error("Loyalty account was not created.");
  await tx.execute(
    sql`select id from loyalty_accounts where id = ${accountId}::uuid for update`,
  );
  await tx
    ...
    .where(
      and(
        eq(loyaltyRedemptions.loyaltyAccountId, accountId),
        eq(loyaltyRedemptions.status, "reserved"),
        lte(loyaltyRedemptions.expiresAt, new Date()),
      ),
    );
  ...
  const [reserved] = await tx
    .select({
      value: sql<number>`coalesce(sum(${loyaltyRedemptions.points}), 0)::integer`,
    })
  ...
  const redemption = calculateLoyaltyRedemption(
    ...
    Math.max(account.balance - reserved.value, 0),
    input.subtotalCents,
    settings,
  );
...
export async function redeemLoyaltyReservation(
  tx: DatabaseTransaction,
  orderId: string,
  now = new Date(),
) {
  ...
  if (!reservation || reservation.status === "redeemed")
    return reservation ? ("duplicate" as const) : ("none" as const);
  if (reservation.status === "released") return "released" as const;
  await tx.execute(
    sql`select id from loyalty_accounts where id = ${reservation.loyaltyAccountId}::uuid for update`,
  );
  const [account] = await tx
    .select({ balance: loyaltyAccounts.balance })
    .from(loyaltyAccounts)
    .where(eq(loyaltyAccounts.id, reservation.loyaltyAccountId))
    .limit(1);
  if (!account || account.balance < reservation.points)
  ...
  if (debit)
    await tx
      ...
      .set({
        balance: sql`${loyaltyAccounts.balance} - ${reservation.points}`,
        updatedAt: now,
      })
      .where(eq(loyaltyAccounts.id, reservation.loyaltyAccountId));
  await tx
    ...
    .where(eq(loyaltyRedemptions.id, reservation.id));
  return debit ? ("redeemed" as const) : ("duplicate" as const);
}

export async function renewLoyaltyReservation(
  tx: DatabaseTransaction,
  orderId: string,
  expiresAt: Date,
  now = new Date(),
  ...
  const [reservation] = await tx
    .select({ id: loyaltyRedemptions.id, status: loyaltyRedemptions.status })
    .from(loyaltyRedemptions)
    .where(eq(loyaltyRedemptions.orderId, orderId))
    .limit(1);
