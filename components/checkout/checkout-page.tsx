    ...
    <section className="py-8 sm:py-12 lg:py-16">
      <Container>
        ...
        <form onSubmit={handleSubmit} onChange={() => { setQuote(null); setQuoteHash(""); setError(null); }} className="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_25rem]">
          ...
          <aside className="lg:sticky lg:top-28"><CheckoutSummary quote={activeQuote} items={items} />{activeQuote ? <div className="mt-4 rounded-xl border-2 border-action bg-mint/40 p-4 text-sm font-bold">{activeQuote.hasPriceChanges ? "El total fue actualizado. Revisalo antes de pagar." : "Total validado. Al continuar reservamos el stock y abrimos Mercado Pago."}</div> : null}{error ? <div role="alert" className="mt-4 rounded-xl border-2 border-red-800 bg-red-50 p-4 text-sm font-bold text-red-900"><p>{error.message}</p>{error.retryAfterSeconds ? <p className="mt-1 text-xs">Podés reintentar en aproximadamente {error.retryAfterSeconds} segundos. El formulario no se pierde.</p> : null}{error.correlationId ? <p className="mt-1 text-xs font-normal">Referencia: {error.correlationId}</p> : null}</div> : null}<button type="submit" disabled={isPending} className="motion-button mt-4 flex min-h-13 w-full items-center justify-center rounded-xl bg-action px-6 py-3 text-base font-black text-white disabled:cursor-wait disabled:opacity-60">{isPending ? "Preparando pago..." : activeQuote ? "Pagar con Mercado Pago" : "Validar total"}</button><Link href="/carrito" className="motion-button mt-4 flex min-h-11 items-center justify-center text-sm font-bold text-action underline decoration-2 underline-offset-4">Volver al carrito</Link></aside>
        </form>
      </Container>
    </section>
  );
