...
export function ProductVisual({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute bottom-6 left-[16%] z-10 h-[62%] w-[24%] min-w-16 -rotate-6 rounded-t-[1.5rem] rounded-b-xl border-2 border-ink/15 shadow-[0_14px_24px_rgba(13,13,13,0.15)] ${style.bottle}`}
      >
      ...
      <div
        className={`absolute right-[18%] bottom-9 h-[55%] w-[24%] min-w-16 rotate-6 rounded-[1rem] border-2 border-ink/15 shadow-[0_14px_24px_rgba(13,13,13,0.15)] ${style.mixer}`}
      >
        <span className="absolute top-[45%] inset-x-0 -rotate-90 text-center text-[0.55rem] font-black tracking-wider sm:text-[0.65rem]">
          {style.mixerLabel}
        </span>
      </div>
      <div className="absolute right-[5%] bottom-4 z-20 h-[42%] w-[26%] rounded-b-2xl border-2 border-white/75 bg-white/25 backdrop-blur-[1px]">
