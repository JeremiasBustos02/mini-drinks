...
const statusMessages = {
  ...
  manual_review: "Estamos revisando el estado del pago. Te avisaremos cuando tengamos una actualización.",
          ...
          <div className="mx-auto max-w-3xl">
            <div className="rounded-[1.75rem] bg-mint/55 p-6 sm:p-10">
              <p className="text-xs font-black tracking-[0.2em] text-action uppercase">Pedido registrado</p>
               <h1 className="mt-3 font-display text-[clamp(2.5rem,8vw,5rem)] leading-[0.9] uppercase">{statusHeadings[result.order.status]}</h1>
            ...
            <div className="mt-6 rounded-[1.5rem] bg-white p-5 sm:p-7">
             ...
             <div className="mt-6 flex flex-wrap gap-3">{result.order.status === "pending_payment" && result.order.mercadoPagoInitPoint && result.order.mercadoPagoPreferenceIsCurrent ? <a href={result.order.mercadoPagoInitPoint} className="motion-button inline-flex min-h-12 items-center rounded-xl bg-action px-6 py-3 font-black text-white">Pagar con Mercado Pago</a> : null}{["payment_rejected", "expired", "cancelled"].includes(result.order.status) ? <Link href="/productos" className="motion-button inline-flex min-h-12 items-center rounded-xl bg-action px-6 py-3 font-black text-white">Volver a elegir</Link> : null}<Link href="/" className="motion-button inline-flex min-h-12 items-center rounded-xl border-2 border-action px-6 py-3 font-black text-action">Volver al inicio</Link></div>
          </div>
        </Container>
      </main>
    </StorefrontShell>
