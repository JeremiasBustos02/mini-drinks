...
const statusMessages = {
  pending_payment:
    "Reservamos tu stock. Completá el pago antes del vencimiento indicado.",
  payment_pending:
    "Pago pendiente. Mercado Pago todavía lo está procesando; no figura como pagado.",
  payment_rejected: "El pago fue rechazado o cancelado y liberamos la reserva.",
  expired:
    "La reserva venció. Volvé al checkout para validar stock e iniciar un pago nuevo.",
  manual_review:
    "Estamos revisando el estado del pago. Te avisaremos cuando tengamos una actualización.",
  paid: "Pago confirmado. Ya recibimos tu pedido.",
...
const statusHeadings = {
  pending_payment: "Completá el pago",
  payment_pending: "Estamos confirmando tu pago",
  payment_rejected: "No se completó el pago",
  expired: "Se venció el tiempo para pagar",
  manual_review: "Estamos revisando tu pago",
  paid: "Pago confirmado",
  preparing: "Estamos preparando tu pedido",
  ready_for_pickup: "Ya podés retirarlo",
        ...
        <Container>
          <div className="mx-auto max-w-3xl">
            <div className="rounded-[1.75rem] bg-mint/55 p-6 sm:p-10">
              ...
              <h1 className="mt-3 font-display text-[clamp(2.5rem,8vw,5rem)] leading-[0.9] uppercase">
                {statusHeadings[result.order.status]}
              </h1>
              <p className="mt-5 text-lg font-bold">
                {shortOrderReference(result.order.publicNumber)}
              </p>
              <p className="mt-1 text-xs text-ink/55">
                Código de referencia: {result.order.publicNumber}
              </p>
              <p className="mt-2 text-sm text-ink/65">
                {statusMessages[result.order.status]}
              </p>
              <span className="mt-5 inline-flex rounded-full bg-white px-4 py-2 text-sm font-black">
                {statusLabels[result.order.status]}
              </span>
              {result.reservation?.isCurrent ? (
                <p className="mt-3 text-xs font-bold text-ink/60">
                  Stock reservado hasta{" "}
                  {result.reservation.expiresAt.toLocaleTimeString("es-AR", {
                    hour: "2-digit",
                    minute: "2-digit",
                  })}
                  .
            ...
            <div className="mt-6 rounded-[1.5rem] bg-white p-5 sm:p-7">
              ...
              <div className="mt-5 space-y-4">
                {result.items.map((item) => {
                  const configuration = item.configurationJson;
                  const components = configuration
                    ? configuration.kind === "preset_combo"
                      ? configuration.components
                      : [
                          ...configuration.baseComponents,
                          ...configuration.extras,
                        ]
                    : [];
                  return (
                    <div key={item.id} className="border-b border-ink/10 pb-4">
              ...
              <div className="mt-5 flex justify-between border-t-2 border-ink pt-4">
                <p className="font-black">Total</p>
                <p className="text-2xl font-black">
                  {formatArsCents(result.order.total)}
                </p>
              </div>
            </div>
            <div className="mt-6 flex flex-wrap gap-3">
              {result.order.status === "pending_payment" &&
              result.order.mercadoPagoInitPoint &&
              result.order.mercadoPagoPreferenceIsCurrent ? (
                <a
                  href={result.order.mercadoPagoInitPoint}
                  className="motion-button inline-flex min-h-12 items-center rounded-xl bg-action px-6 py-3 font-black text-white"
                >
                  Pagar con Mercado Pago
                </a>
              ) : null}
              {["payment_rejected", "expired", "cancelled"].includes(
                result.order.status,
              ) ? (
                <Link
                  href="/productos"
              ...
              <Link
                href="/"
                className="motion-button inline-flex min-h-12 items-center rounded-xl border-2 border-action px-6 py-3 font-black text-action"
              >
                Volver al inicio
              </Link>
            </div>
